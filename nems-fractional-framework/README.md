...
Sou Luiz Tiago Wilcke. Este código simula um nanoeletromecânico (NEMS) dirigido por movimento browniano fracionário e recupera a deriva por projeção ortogonal. Também calcula uma U-estatística degenerada de segunda ordem como diagnóstico de dispersão.
...
$$
\Sigma = LL^\top, \qquad B^H = Lz, \quad z \sim \mathcal{N}(0,I).
$$
...
$$
X_{k+1} = X_k + b_0(X_k)\Delta t_k + \sigma\,\Delta B^H_k,
$$
...
$$
\hat{\Psi}_m = \frac{1}{NT}\sum_{i=1}^{N}\sum_{k=0}^{n-1} \varphi(X_{i,k})\varphi(X_{i,k})^\top \Delta t,
$$

$$
\hat{Z}_m = \frac{1}{NT}\sum_{i=1}^{N}\sum_{k=0}^{n-1} \varphi(X_{i,k})\,\Delta X_{i,k}.
$$

O sistema regularizado

$$
\bigl(\hat{\Psi}_m + \varepsilon I\bigr)\hat{\theta}_m = \hat{Z}_m
$$

é resolvido e a deriva recuperada é

$$
\hat{b}_m(x) = \sum_{j=0}^{m-1}\hat{\theta}_j\,\varphi_j(x).
$$

O menor autovalor de \( \hat{\Psi}_m \) é impresso como diagnóstico de estabilidade espectral.
...
$$
H_2(X^i,X^j) = \frac{1}{n}\sum_{k=0}^{n}\bigl(X^i_k - X^j_k\bigr)^2
$$

e a U-estatística amostral

$$
U_N = \binom{N'}{2}^{-1}\sum_{1\le i < j\le N'} H_2(X^i,X^j),
$$
...
O script gera as trajetórias fracionárias, ajusta a projeção, imprime o autovalor mínimo e a U-estatística, e mostra o gráfico de \( b_0 \) versus \( \hat{b}_m \).
...
| `EstimadorProjecaoMalliavinNEMS` | Monta \( \hat{\Psi}_m \), \( \hat{Z}_m \) e reconstrói a deriva |
